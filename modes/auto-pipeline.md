...
Run exactly the same as the `oferta` mode (read `modes/oferta.md` for all blocks A-F + Block G Posting Legitimacy).
...
Source of truth is `data/jobs.json`. Update the job in place (matched by URL): set `fitScore`, `decision`, `report` (path to the saved report), and `appNum` (next integer), plus any notes. Then run `node scripts/gen-applications-md.mjs` to refresh `data/applications.md` (with all columns, including Report and PDF as ✅).
