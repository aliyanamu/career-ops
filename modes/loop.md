...
Run `node scripts/scan.mjs` (zero-token ATS scan; appends fresh URLs to `data/pipeline.md`). If `discovery.enabled` is set in `portals.yml`, the same run adds opt-in Firecrawl broad-discovery results (Phase 3).
...
- Then run `node scripts/gen-applications-md.mjs` to refresh `applications.md`.

### 3. STOP — review checkpoint
Run `node scripts/loop.mjs`. It prints the **review queue**: scored roles with `fitScore ≥ 4.0` **or** any user-added role (any score), that the human hasn't actioned yet. `scripts/loop.mjs` owns this selection rule — do not re-derive it here.
...
Run `node scripts/loop.mjs draft` → approved roles with no cover letter yet. For each:
...
On-demand: the user says "run the loop" / `loop`. Not scheduled. The deterministic selection is testable: `node scripts/loop.mjs --self-check`.
