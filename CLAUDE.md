...
- Set up the private fork `aliyanamu/career-ops`, rewired remotes, committed `Job Hunting Progress.xlsx` into the repo, added this CLAUDE.md.
...
5. Update the `Prep` sheet in `Job Hunting Progress.xlsx` via `openpyxl` — add a new row, fill all columns including AI-disclaimer flag.
