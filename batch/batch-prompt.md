...
| scripts/generate-pdf.mjs | `scripts/generate-pdf.mjs` | For PDF |
...
**IMPORTANT:** The TSV order has status BEFORE score (col 5→status, col 6→score). In applications.md the order is reversed (col 5→score, col 6→status). scripts/merge-tracker.mjs handles the conversion.
