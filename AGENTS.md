...
Personal fork, maintained by Hana Aliyah Mufidah for her own job search. Detached from upstream — updates are not pulled from the original repo.

Honorable mention: the original career-ops system was built by [santifer](https://github.com/santifer/career-ops) (MIT). This fork adapts it and does not track its releases.
...
- `career-direction/*` (active direction + requirements log — biases scan/scoring)
- `data/*`, `reports/*`, `output/*`, `interview-prep/*`

**System Layer (auto-updatable, DON'T put user data here):**
- `modes/_shared.md`, `modes/oferta.md`, all other modes
- `AGENTS.md`, `CLAUDE.md`, `*.mjs` scripts, `tracker/*`, `templates/*`, `batch/*`
...
**When NOT to:** If the user applies to English-language roles, even at Japanese companies, use the default English modes.
...
| Wants a cover letter for a role | `cover` |
...
0. **Scan auto-imports.** `scripts/scan.mjs` appends new roles straight into `data/jobs.json` as unevaluated leads (`fitScore: ""`, `decision: "pending"`, `preparation`/`application` null), deduped by URL. There is no manual import step. So when you evaluate, the job usually **already exists** — find it by URL and update it **in place**. NEVER create a second row for a URL/company+role already in `jobs.json`.
1. **To add/update an evaluated offer:** locate the existing job by URL (scan pre-created it) or add one if truly new, then set `fitScore`, `decision`, `report` (path to the report), `appNum` (next integer), and `notes`; set the `application` sub-object when applied. Then run `node scripts/gen-applications-md.mjs` to refresh `applications.md`. A job appears in `applications.md` once it has a `report` field.
