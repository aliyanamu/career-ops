...
- **Honest framing:** "I haven't used Elixir. I've shipped a lot of correctness-critical backend, and functional/immutable thinking is already how I reason about reliable systems." (Do not claim to be currently learning it unless that becomes true.)
- **Plan if hired / if asked how you'd approach it:** would start with Elixir + Phoenix basics (pattern matching, GenServer/supervision trees, Ecto for Postgres); the OTP worker/supervision model maps to the forward/backward/cleanup job design already built imperatively.
...
- **Nespay (Apr 2025–present):** NestJS/TS backend (the payments platform + dashboard backend that consumes the indexer webhooks); wallet transactions, invoicing, Xellar/Privy; KYC (Sumsub, Kredibel); AWS (EC2, IAM) + GitHub Actions CI/CD; core architecture ownership.
- **Block indexer — built in Go (relay service):** forward indexer (real-time block scan) + backward indexer (gap fill, configurable to re-scan >7 days after an outage) + periodic cleanup job (prunes data older than ~3 days to keep **PostgreSQL** from bloating); Redis dedup so each tx reports once; notifies the NestJS backend when a tx is paid. Goal: lean middle service, not long-term storage. (Indexer = Go; consumer backend = NestJS. Don't mix them up.)
...
The block indexer I work on at Nespay, in Go. I did not start it, I took it over and extended it. Its job is to sit in the middle, watch the chain, and let our backend know when a transaction has actually been paid. The tricky part was that the first version tried to hold onto everything it scanned, and that got expensive fast. Postgres kept bloating with data we didn't really need. So I reshaped it around what the service actually needs to do. A forward indexer reads new blocks as they come in, a backward indexer fills any gaps and is configurable enough to re-scan more than a week back if we ever have an outage, and a periodic job clears out anything older than a few days since we don't need to keep it around. I'm proud of it because it does one job well and stays cheap to run. It relays what matters, forgets what it doesn't, and holds up even when something upstream goes down. It's the clearest case of me taking over a system, improving it, and keeping it simple on purpose.

**8. Do you use AI coding assistants (Claude, Copilot, etc.)? If yes, a specific scenario where you rejected or significantly rewrote the output. What did the AI miss, and how did you catch it?**
Yes, I use Claude every day, mostly for brainstorming, planning, drafting docs, and implementation. The thing I reject or re-iterate on most is when it doesn't admit it doesn't know something and just assumes instead. That's often worse than a gap, because the assumption comes out sounding confident and slips through if you're not paying attention. What it misses, almost always, is context it was never actually given, so it fills the hole with a guess. The way I catch and prevent it is mostly in the setup. From the first time I work on something, I pool the important context in one place, specific folders and files. I use compound-engineering to keep a neat cycle: drafts go in docs/brainstorms and docs/plans, and then I actually review them and sync the related docs, since the business side and the technical side drift apart fast. I'll also ask it to draw a mermaid diagram of what it thinks we're building, which makes it obvious where its picture and mine don't line up. And I end almost every prompt with "if anything is unclear, ask me," which does a surprising amount of work. It stops the model from filling gaps with guesses and keeps it in scope. So instead of one big rewrite at the end, I catch the wrong assumptions early, where they're cheap to fix.