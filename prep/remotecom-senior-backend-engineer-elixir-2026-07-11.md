...
Two things pull me toward Remote. First, the mission. I'm based in Indonesia, and Remote is the reason a company can hire someone like me anywhere without the usual compliance mess. I've felt that friction from the candidate side, so helping build the thing that removes it actually means something to me. Second, the way Remote works. It's fully async and high ownership, without a lot of ceremony, and that's already how I operate. I own my payments backend end to end and manage my own time without daily standups. The stack fits me too: React and Next.js, PostgreSQL, AWS, CI/CD. Elixir is the one new piece, and I'm genuinely excited to pick it up. (GitHub: github.com/aliyanamu)

**Q15. You've seen our values page. What resonates most?**
Yes, I read it. Ownership is the one that resonates most. My best work happens when I own the outcome instead of just a ticket. With our payments indexer I noticed the single detection loop was missing deposits and settling at the wrong time, and instead of just flagging it I redesigned it into separate forward, backfill, and confirmation workers so settlement was correct and could recover from failure. Kindness is a close second, and I like that it's your first value. I try to run code reviews as questions rather than verdicts. I'd rather ask what someone was going for than assume they made a mistake.

**Q17. Elixir experience, how you'd learn it, what appeals about FP?**
I'll be honest. I haven't shipped Elixir in production. My backend work is in Node.js, NestJS, and TypeScript. But the way Elixir thinks about problems is close to how I already build. My payments indexer is basically a set of independent, message passing workers with supervision and recovery, which is the same idea as OTP, GenServer, and supervision trees. So that's where I'd start. I'd pick up Elixir and pattern matching first, then Phoenix and Ecto, since those map onto the Postgres and API work I do now. Then I'd rebuild a small version of that indexer in idiomatic OTP so I'm learning the language on a problem I already understand. What draws me to functional programming is immutability and being explicit. You get fewer hidden state bugs and correctness you can actually reason about, which matters a lot when you're moving real money. I'm happy to do a take home or a pairing session in Elixir so you can see how fast I ramp.

**Q19. A product or project you led through collaboration with stakeholders, product, and design.**
At ION I led improvements to the employee time off and work timeline system on our work management platform. It started as a product problem. Managers were drowning in one by one approvals, and time off ignored public holidays and team objectives, so people's plans kept clashing. I worked with product to pin down the real need, with design on how bulk approvals and the timeline should feel, and with HR and managers to check the edge cases. Then I built bulk approvals and connected time off data with public holidays and objectives so the timeline matched reality. It turned a slow, error prone process into something managers could act on at a glance. What I took from it is that the engineering was the easy part. The value came from getting the spec right with the people who aren't technical first.

**Q20. Proudest achievement from your last two years. Why?**
The multi chain settlement indexer I built at Nespay. Our B2B payments platform settles invoices from on chain USDT deposits across five chains, and a naive single detection loop was missing deposits and settling at the wrong time. I redesigned it into three workers that coordinate. A forward worker polls recent blocks every five seconds or so, a backfill worker closes gaps, and a confirmation worker waits for each chain's finality before anything settles. It sits behind an RPC layer, uses Redis for dedup and Postgres for crash recovery, and fires webhooks that match each transfer to the open invoice by amount. I'm proud of it because it moves real money and it's correct. Verification and recovery are built in, and it went from something fragile to something I actually trust. It's also my clearest example of owning a system end to end and improving it step by step.

**Q21. Do you use AI assistants? A time you rejected or rewrote AI output, what it missed, how you caught it.**
(Replace with a true incident if this isn't exactly right, but it fits your indexer work.)
Yes, I use AI assistants like Claude and Copilot every day to move faster. Here's a specific time I threw the output away. While building the settlement indexer, I asked an assistant to draft the deposit detection logic, and it treated a transaction as settled the moment it showed up in a block. That's wrong when you're moving money. It ignored chain finality and reorgs, so a deposit could look settled and then vanish when the chain reorganized. I caught it because I know how finality works. Different chains need different confirmation depths, and you can't settle on first sight. I rewrote it so detection and confirmation are separate steps. Detect when the transaction appears, but only settle after the chain specific confirmations. That became the confirmation worker. The way I see it, AI is great for scaffolding, but I stay the gatekeeper on anything that touches money or correctness, because it writes plausible code, not code that respects the domain's real failure modes.
